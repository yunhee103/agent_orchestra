...
3. **외부 연동 대상이 특정되지 않은 것 — 절대 추측 금지.** 인증, 결제, 알림, 클라우드, 스토리지 등 제3자(Third-Party) 외부 시스템과의 모든 접점에서 구체적인 '제품명/벤더명'이 명시되지 않았다면 절대로 임의 선택하지 마라. 
...
발견 사항은 담당 개발자가 그대로 반영해 재작성하므로, suggestion은 실행
가능한 구체적 지시로 써라(무엇을 삭제/교체할지). file은 제공된 파일 경로를
그대로 써라. 발견이 없으면 findings를 빈 배열로.
